...
* Global Variables: PWM_FREQUENCY
...
int main(void) {
	...
	volatile uint8_t ui8AdjustR;//Red intensity
	volatile uint8_t ui8AdjustG;//Greeen intensity
	volatile uint8_t ui8AdjustB;//Blue intensity
	volatile int t;
	...
	while (1) {
		...
		PWMPulseWidthSet(PWM1_BASE, PWM_OUT_5,
				1 + ui8AdjustR * ui32Load / 1000);
		PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6,
				1 + ui8AdjustB * ui32Load / 1000);
		PWMPulseWidthSet(PWM1_BASE, PWM_OUT_7,
				1 + ui8AdjustG * ui32Load / 1000);
		t++;
		SysCtlDelay(m);
	}
