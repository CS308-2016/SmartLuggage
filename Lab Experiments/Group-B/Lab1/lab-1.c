...
int main(void)
{
	...
	// Cycle through Red, Green and Blue LEDs
		int32_t D0 = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4);
		int32_t D1 = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0);
		if(!D1){
			while(! D1){
				D1 = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_0);
			}
		...
		while(! D0){
			D0 = GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4);
		}
